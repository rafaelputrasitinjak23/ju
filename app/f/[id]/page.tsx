...
import { Cloud, Download, Eye, Lock, FileText, Film, Music, Image as ImageIcon, File, Copy, ArrowLeft, RefreshCw, AlertCircle, QrCode, X, Smartphone, FileCode } from 'lucide-react';
...
export default function FilePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const isTextFile = Boolean(
    file && (
      ...
      file.originalName.match(/\.(txt|js|json|html|css|py|md|xml|ts|tsx|jsx|sh|c|cpp|h|java)$/i)
    )
  );

  useEffect(() => {
    if (file && isUnlocked && isTextFile && textContent === null && !loadingText) {
      let ignore = false;
      const loadText = async () => {
        setLoadingText(true);
        try {
          const rawAccessUrl = file.isProtected
            ? `${file.rawUrl}?pwd=${encodeURIComponent(password)}`
            : file.rawUrl;
          const res = await fetch(rawAccessUrl);
          if (res.ok) {
            const txt = await res.text();
            if (!ignore) setTextContent(txt);
          }
        } catch (err) {
          console.error('Failed to load text content:', err);
        } finally {
          if (!ignore) setLoadingText(false);
        }
      };
      loadText();
      return () => {
  ...
  }, [file, isUnlocked, isTextFile, textContent, loadingText, password]);
  ...
  const getFileIcon = (mime: string, name: string) => {
    if (mime.startsWith('image/')) return <ImageIcon className="w-8 h-8 text-sky-400" />;
    if (mime.startsWith('video/')) return <Film className="w-8 h-8 text-indigo-400" />;
    if (mime.startsWith('audio/')) return <Music className="w-8 h-8 text-purple-400" />;
    if (mime.includes('pdf')) return <FileText className="w-8 h-8 text-emerald-400" />;
    if (mime.includes('text') || name.match(/\.(txt|js|json|html|css|py|md|xml|ts|tsx)$/i)) {
  ...
  if (loading) {
    return (
      <div className="min-h-screen text-zinc-100 flex flex-col items-center justify-center p-4">
        <div className="bg-zinc-900/90 backdrop-blur-md border border-zinc-800/90 rounded-3xl p-8 max-w-sm w-full text-center space-y-3 shadow-2xl animate-fade-in">
          <RefreshCw className="w-8 h-8 animate-spin text-purple-400 mx-auto" />
          <p className="text-xs font-mono-code text-zinc-300">Menghubungkan ke Cloud Storage...</p>
        </div>
      </div>
    );
  }
  ...
  if (error || !file) {
    return (
      <div className="min-h-screen text-zinc-100 flex flex-col items-center justify-center p-4">
        <div className="bg-zinc-900/90 backdrop-blur-md border border-rose-900/60 rounded-3xl p-8 max-w-md w-full text-center space-y-4 shadow-2xl animate-fade-in">
          <AlertCircle className="w-12 h-12 text-rose-400 mx-auto" />
          <h2 className="text-xl font-bold font-serif-elegant text-white">File Tidak Ditemukan</h2>
          <p className="text-xs text-zinc-400 font-outfit leading-relaxed">
            {error || 'Berkas yang Anda cari mungkin telah dihapus atau ID file tidak valid.'}
          </p>
          <Link
            href="/"
            className="inline-flex items-center gap-2 px-5 py-2.5 bg-zinc-100 hover:bg-white text-zinc-950 text-xs font-bold rounded-xl transition-all mt-2 shadow-lg shadow-white/10"
          >
  ...
  return (
    <div className="min-h-screen text-zinc-100 font-['Outfit',sans-serif] flex flex-col selection:bg-zinc-800 selection:text-zinc-100">
      ...
      <header className="bg-zinc-900/90 backdrop-blur-md border-b border-zinc-800/80 px-4 lg:px-12 py-3.5 sticky top-0 z-30 shadow-lg shadow-black/20">
        <div className="max-w-7xl mx-auto flex items-center justify-between gap-4">
          ...
          <Link
            href="/"
            className="text-xs text-zinc-400 hover:text-zinc-100 flex items-center gap-1.5 transition-colors font-medium bg-zinc-950 px-3.5 py-2 rounded-xl border border-zinc-800"
          >
      ...
      <main className="flex-1 max-w-4xl w-full mx-auto p-4 md:p-8 space-y-6">
        ...
        {file.isProtected && !isUnlocked ? (
          <div className="bg-zinc-900 border border-amber-900/60 rounded-3xl p-8 max-w-md mx-auto text-center space-y-6 shadow-2xl animate-fade-in">
            ...
            <form onSubmit={handleUnlock} className="space-y-4">
              <input
                ...
                className="w-full bg-zinc-950 border border-zinc-800 rounded-xl px-4 py-3 text-sm text-center text-zinc-100 placeholder:text-zinc-500 focus:outline-none focus:border-amber-500 font-mono-code"
              />

              {unlockError && (
                <p className="text-xs text-rose-300 bg-rose-950/60 p-2.5 rounded-lg border border-rose-800/80 font-mono-code">
                  {unlockError}
                </p>
              )}
              ...
              <button
                type="submit"
                disabled={unlocking}
                className="w-full py-3 bg-zinc-100 hover:bg-white text-zinc-950 font-bold text-sm rounded-xl shadow-lg transition-all flex items-center justify-center gap-2"
              >
                {unlocking ? <RefreshCw className="w-4 h-4 animate-spin text-zinc-950" /> : <Lock className="w-4 h-4 text-zinc-950" />}
          ...
          <div className="space-y-6 animate-fade-in">
            ...
            <div className="bg-zinc-900/90 backdrop-blur-md border border-zinc-800/90 rounded-3xl p-6 md:p-8 shadow-2xl space-y-6">
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-zinc-800 pb-6">
                <div className="flex items-start gap-4 min-w-0">
                  <div className="p-3.5 bg-zinc-950 rounded-2xl border border-zinc-800 shrink-0">
                  ...
                  <div className="min-w-0">
                    ...
                    <div className="flex flex-wrap items-center gap-3 mt-1.5 text-xs font-mono-code text-zinc-400">
                ...
                <div className="flex items-center gap-2.5 shrink-0">
                  <button
                    ...
                    className="px-4 py-3.5 bg-zinc-800 hover:bg-zinc-700 text-zinc-200 font-semibold text-xs rounded-2xl border border-zinc-700 transition-all flex items-center gap-2 shadow-xs"
                  ...
                  <a
                    ...
                    download
                    className="px-6 py-3.5 bg-zinc-100 hover:bg-white text-zinc-950 font-bold text-sm rounded-2xl shadow-lg shadow-white/10 transition-all flex items-center justify-center gap-2.5"
                  >
              ...
              {/* Stats & Cloud Storage Badge */}
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                <div className="bg-zinc-950 p-3.5 rounded-2xl border border-zinc-800 flex items-center gap-3">
                  <Eye className="w-5 h-5 text-sky-400 shrink-0" />
                  <div>
                    <span className="block text-[10px] text-zinc-500 uppercase font-mono-code">Dilihat</span>
                    <span className="text-sm font-bold text-zinc-100 font-mono-code">{file.viewsCount} kali</span>
                  </div>
                </div>
                ...
                <div className="bg-zinc-950 p-3.5 rounded-2xl border border-zinc-800 flex items-center gap-3">
                  ...
                  <div>
                    <span className="block text-[10px] text-zinc-500 uppercase font-mono-code">Diunduh</span>
                    <span className="text-sm font-bold text-zinc-100 font-mono-code">{file.downloadsCount} kali</span>
                  </div>
                </div>
                ...
                <div className="bg-zinc-950 p-3.5 rounded-2xl border border-zinc-800 flex items-center gap-3">
                  ...
                  <div className="min-w-0">
                    <span className="block text-[10px] text-zinc-500 uppercase font-mono-code">Cloud Storage</span>
                    <span className="text-xs font-bold text-zinc-100 font-mono-code truncate block">
            ...
            <div className="bg-zinc-900 border border-zinc-800 rounded-3xl p-6 shadow-2xl space-y-4">
              <div className="flex items-center justify-between">
                ...
                {isTextFile && textContent && (
                  <button
                    onClick={() => copyToClipboard(textContent, 'text_content')}
                    className="flex items-center gap-1.5 px-3 py-1.5 bg-zinc-800 hover:bg-zinc-700 text-zinc-200 text-xs font-medium rounded-xl border border-zinc-700 transition-all"
                  >
                    <Copy className="w-3.5 h-3.5 text-zinc-400" />
                    <span>{copiedType === 'text_content' ? 'Tersalin!' : 'Salin Teks'}</span>
                  </button>
                )}
              </div>

              <div className="bg-zinc-950 rounded-2xl border border-zinc-800 p-4 min-h-[220px] flex items-center justify-center overflow-hidden">
                {file.mimeType.startsWith('image/') ? (
                  ...
                  <iframe
                    src={rawAccessUrl}
                    className="w-full h-[500px] rounded-xl border border-zinc-800 bg-white"
                  />
                ) : isTextFile ? (
                  <div className="w-full space-y-2">
                    {loadingText ? (
                      <div className="flex items-center justify-center py-12 text-zinc-400 gap-2">
                        ...
                        <span className="text-xs font-mono-code">Memuat isi teks mentah...</span>
                      </div>
                    ) : (
                      <pre className="w-full max-h-[500px] overflow-auto bg-zinc-950 p-4 rounded-xl border border-zinc-800/80 text-xs font-mono-code text-zinc-200 whitespace-pre-wrap leading-relaxed select-text">
                        {textContent || 'Teks kosong atau tidak dapat dimuat.'}
                      </pre>
                    )}
                  </div>
                ) : (
                  <div className="text-center py-8 space-y-3">
                    ...
                    <p className="text-xs text-zinc-400 font-outfit max-w-xs mx-auto">
            ...
            <div className="bg-zinc-900 border border-zinc-800 rounded-3xl p-6 shadow-2xl space-y-4">
              <div className="flex items-center justify-between">
                ...
                <button
                  onClick={() => setShowQrModal(true)}
                  className="px-3 py-1.5 bg-zinc-800 hover:bg-zinc-700 text-zinc-200 text-xs font-semibold rounded-xl border border-zinc-700 transition-all flex items-center gap-1.5"
                >
          ...
          <div className="fixed inset-0 z-50 bg-zinc-950/80 backdrop-blur-sm flex items-center justify-center p-4 animate-fade-in">
            <div className="bg-zinc-900 border border-zinc-800 rounded-3xl p-6 md:p-8 max-w-sm w-full space-y-6 shadow-2xl relative text-center">
              <button
                ...
                className="absolute top-4 right-4 p-2 text-zinc-400 hover:text-zinc-100 bg-zinc-800 hover:bg-zinc-700 rounded-xl border border-zinc-700 transition-all"
                title="Tutup Modal"
              >
                <X className="w-4 h-4" />
              </button>
              ...
              <div className="space-y-1 pt-2">
                <div className="w-12 h-12 rounded-2xl bg-zinc-950 border border-zinc-800 flex items-center justify-center text-zinc-200 mx-auto mb-3">
              ...
              <div className="bg-white p-5 rounded-2xl inline-block border-2 border-zinc-700 shadow-sm mx-auto">
                <QRCodeSVG
              ...
              <div className="space-y-3 pt-1">
                <div className="bg-zinc-950 p-3 rounded-xl border border-zinc-800 text-left">
                  ...
                  <p className="text-xs font-semibold text-zinc-200 truncate font-outfit">{file.originalName}</p>
                  <p className="text-[10px] text-zinc-400 font-mono-code mt-0.5">{formatBytes(file.size)}</p>
                </div>
